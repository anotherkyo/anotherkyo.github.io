/* chaos_v7.6.js
 * - v7.5 기반
 * - 고유카드 복제 / 번뜩임 / 신번뜩임 규칙 수정
 * - 복제 점수 누적 방식(1~N회) 반영
 * - 로그에 PT 변화량 표시 (스타일 B: 메시지 뒤 괄호)
 */
...
function logWithPt(pl, baseMsg) {
  const before = pl._lastTotal || 0;
  updatePlayerGauge(pl); // 내부에서 pl._lastTotal 갱신
  const after = pl._lastTotal || 0;
...
function calcCardContribution(card) {
  ...
  if (
    ...
    // unique_clone 은 기본 0pt (복제 개념 전용)
    // 번뜩임 / 신번뜩임 보정
    if (card.state === "spark") {
      // 일반 카드: +10, 고유 복제: 0
...
function updatePlayerGauge(pl) {
  ...
  // 추가 카드(중립/몬스터/금기/고유 복제)들의 복제 수와 번뜩/신번 보정
  pl.cards.forEach((c) => {
    if (c.dupCount && c.dupCount > 0) {
      dupCopies += c.dupCount;

      if (c.state === "spark") {
        // 일반카드 복제: 복제본마다 +10
        if (c.type !== "unique_clone") {
          dupSparkPt += 10 * c.dupCount;
        }
      } else if (c.state === "newspark") {
        // 신번은 일반/고유 모두 복제본마다 +20
        dupSparkPt += 20 * c.dupCount;
      }
    }
  });
...
function changePlayerCharacter(pl, newId) {
  ...
  // 캐릭터 변경 자체는 PT 변화 크기가 크므로 로그 + PT 표시
