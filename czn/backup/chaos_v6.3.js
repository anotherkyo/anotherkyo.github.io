 ...
 * - 고유일반1~3, 고유신화1은 번뜩임 UI 표시 없음
 * - 캐릭터별 상한/총 pt 텍스트 삭제, 게이지에만 숫자 표시
 * - 고유카드 변환 버튼 제거, 변환 체크박스(토글)로 통일
 * - 티어 셀렉트 우측에 초기화 버튼 추가 (티어 0으로 리셋)
 */
...
function calcCardContribution(card) {
  ...
  if (card.type === "taboo") {
    ...
    if (card.type === "unique_clone") {
      // 고유카드 복제의 기본 pt는 중립 카드와 동일하게 20pt로 처리
      total += BASE.neutral;
    }
