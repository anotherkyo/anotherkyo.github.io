/* chaos_v7.3.js
 * - 상단 레이아웃: 초상화 왼쪽 / 캐릭터 선택 + 카드 추가 오른쪽
 * - 일반/복제/금기: 변환 없음, 제거 상태 있음
 * - 고유카드: 제거 가능, 변환 가능, 신번뜩만(+20) 유지
 * - 제거 PT = 제거회차PT + (고유 ? 20 : 0) + (spark/newspark ? 20 : 0)
 * - 중립/몬스터/금기/복제: 번뜩=+10, 신번뜩=+30
 * - 티어 공용, 게이지에 "현재 / 상한 pt (퍼센트%)" 표시
 */
...
function updatePlayerGauge(pl) {
  ...
  removedCards.forEach((rc) => {
    ...
    const isSpark = rc.state === "spark" || rc.state === "newspark";
    const uniqueBonus = rc.isUnique ? 20 : 0;
    const sparkBonus = isSpark ? 20 : 0;
